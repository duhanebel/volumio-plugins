...
AmpSwitchController.prototype.getUIConfig = function() {
    ...
    .then(function(uiconf)
          ...
          uiconf.sections[0].content[0].value.label = self.config.get('port').toString();
					...
					uiconf.sections[0].content[2].value = self.config.get('latched');
					uiconf.sections[0].content[3].value = self.config.get('on_pulse_width')
					uiconf.sections[0].content[4].value = self.config.get('off_pulse_width')
          defer.resolve(uiconf);
          })
    .fail(function()
          {
self.logger.ASdebug('gergergergergergerge')
