...
ControllerPlanetRockRadio.prototype.handleBrowseUri = function (curUri) {
  ...
  if (curUri.startsWith('planetradio/')) {
    ...
    axios
      .get('https://listenapi.planetradio.co.uk/api9.2/initweb/' + stationCode)
      .then(function (response) {
        ...
        const item = {
          ...
          albumart: station.stationSquareLogo || '/albumart?sourceicon=music_service/planetrock_multi_radio/assets/planetrock_multi_radio.webp',
        };
        defer.resolve({
          navigation: {
...
ControllerPlanetRockRadio.prototype.authenticate = function () {
  ...
  axios
    .post('https://account.planetradio.co.uk/ajax/process-account/')
    .then(response => {
      ...
      try {
        ...
        var loginData = {
          ...
          passwordfield: encodeURIComponent(password)
            .replace(/!/g, '%21')
            .replace(/'/g, '%27')
            .replace(/\(/g, '%28')
            .replace(/\)/g, '%29')
...
ControllerPlanetRockRadio.prototype.getRootContent = function () {
  ...
  axios
    .get('https://listenapi.planetradio.co.uk/api9.2/initweb/pln')
    .then(function (response) {
      if (!response.data) {
...
ControllerPlanetRockRadio.prototype.streamHlsSegments = function (segments, res) {
  ...
  const streamNextSegment = () => {
    ...
    if (segment.metadataUrl && segment.metadataUrl !== 'https://listenapi.planetradio.co.uk/api9.2/eventdata/-1' && segment.metadataUrl !== lastMetadataUrl) {
      ...
      self
        .fetchMetadataFromUrl(segment.metadataUrl)
        .then(metadataData => {
          if (metadataData) {
            const metadata = self.createMetadataObject(metadataData.eventSongTitle, metadataData.eventSongArtist, '', metadataData.eventImageUrl);
            self.updateMetadata(metadata);
          }
        })
        .catch(error => {
...
ControllerPlanetRockRadio.prototype.refreshHlsPlaylist = function (playlistUrl, currentSegments, res, continueCallback) {
  ...
  self
    .fetchM3u8Playlist(authenticatedPlaylistUrl)
    .then(newSegments => {
